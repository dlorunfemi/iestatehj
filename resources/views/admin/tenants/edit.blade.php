...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('global.tenant.title_singular') }}
    </div>
            ...
            <div class="form-group {{ $errors->has('apartment_id') ? 'has-error' : '' }}">
                ...
                <select name="apartment_id" id="apartment" class="form-control select2">
                    @foreach($apartments as $id => $apartment)
                        <option value="{{ $id }}" {{ (isset($tenant) && $tenant->apartment ? $tenant->apartment->id : old('apartment_id')) == $id ? 'selected' : '' }}>{{ $apartment }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('is_active') ? 'has-error' : '' }}">
                ...
                <select id="is_active" name="is_active" class="form-control">
                    ...
                    @foreach(App\Tenant::IS_ACTIVE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_active', $tenant->is_active) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            {{-- <div class="form-group {{ $errors->has('updated_by_id') ? 'has-error' : '' }}">
                ...
                <select name="updated_by_id" id="updated_by" class="form-control select2">
                    @foreach($updated_bies as $id => $updated_by)
                        <option value="{{ $id }}" {{ (isset($tenant) && $tenant->updated_by ? $tenant->updated_by->id : old('updated_by_id')) == $id ? 'selected' : '' }}>{{ $updated_by }}</option>
                    @endforeach
                </select>
