    ...
    <div class="card-body">
        <form action="{{ route("admin.tenants.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('created_by_id') ? 'has-error' : '' }}">
                <label for="created_by">{{ trans('global.tenant.fields.created_by') }}*</label>
                <select name="created_by_id" id="created_by" class="form-control select2">
                    @foreach($created_bies as $id => $created_by)
                        <option value="{{ $id }}" {{ (isset($tenant) && $tenant->created_by ? $tenant->created_by->id : old('created_by_id')) == $id ? 'selected' : '' }}>{{ $created_by }}</option>
                    @endforeach
                </select>
